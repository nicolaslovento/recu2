 ...
 */
 ...
 ArrayList* al_Filter(ArrayList* eArray,int (*punteroAfuncion)(void* puntero))
 {
     ArrayList* newArray=NULL;
     if(eArray!=NULL && punteroAfuncion!=NULL)
     {
         int len,i;
        void* punteroAux;
         ...
         for(i=0;i<len;i++)
         {
             punteroAux=al_get(eArray,i);
             if(punteroAfuncion(punteroAux)==1)
             {
                 al_add(newArray,punteroAux);
             }
         }
